...
describe('AssetService Unit & Integration Tests', () => {
    ...
    beforeEach(() => {
        ...
        mockStorage = {
            createUploadUrl: jest.fn().mockResolvedValue('https://r2.test/upload-presigned-url'),
            createDownloadUrl: jest.fn().mockResolvedValue('https://r2.test/download-presigned-url'),
            ...
            getPublicUrl: jest.fn((params) => `https://cdn.example.com/${params.key}`),
        };
    ...
    describe('createUploadUrl', () => {
        test('generates presigned upload URL and pending asset record', async () => {
            ...
            expect(result.storageKey).toMatch(/^games\/cyber-clash\/files\/v1\.0\.0\/[a-f0-9]{8}-game-build\.zip$/);
        ...
        test('prevents overwriting an existing ready build version', async () => {
            ...
            await expect(assetService.createUploadUrl({
                gameId: 'cyber-clash',
    ...
    describe('completeUpload', () => {
        test('verifies object exists in storage and marks ready', async () => {
            ...
            expect(mockStorage.objectExists).toHaveBeenCalledWith({ key: mockAsset.storageKey });
        ...
        test('marks asset failed and throws error when object missing in storage', async () => {
            ...
            await expect(assetService.completeUpload({
    ...
    describe('getDownloadUrl & Asset Access', () => {
        test('returns public CDN URL for public ready asset', async () => {
            ...
            expect(result.url).toBe('https://cdn.example.com/games/cyber-clash/images/banner.webp');
        });
        ...
        test('denies unauthenticated/non-admin user access to private asset', async () => {
            ...
            await expect(assetService.getDownloadUrl({
    ...
    describe('deleteAsset', () => {
        test('deletes object from storage and removes database record', async () => {
            ...
            expect(mockStorage.deleteObject).toHaveBeenCalledWith({ key: mockAsset.storageKey });
