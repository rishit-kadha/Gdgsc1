...
const { generateStorageKey, sanitizeFilename, sanitizeVersion } = require('../utils/storageKey');
...
class AssetService {
     ...
     */
    validateUploadInput({ type, category, filename, contentType, size, version }) {
        if (!filename) {
            throw new ApiError(400, 'VALIDATION_ERROR', 'filename is required');
        }

        if (!type || !storageConfig.types.includes(type)) {
            throw new ApiError(400, 'INVALID_ASSET_TYPE', `type must be one of: ${storageConfig.types.join(', ')}`);
        }

        if (!category || !storageConfig.categories.includes(category)) {
            throw new ApiError(400, 'INVALID_ASSET_CATEGORY', `category must be one of: ${storageConfig.categories.join(', ')}`);
        }
    ...
    async resolveGame(gameIdentifier) {
        let game = null;
        if (typeof gameIdentifier === 'string' && gameIdentifier.match(/^[0-9a-fA-F]{24}$/)) {
        ...
        if (!game) {
            throw new ApiError(404, 'GAME_NOT_FOUND', `Game '${gameIdentifier}' not found`);
        }
        return game;
    }
     ...
     */
    async createUploadUrl({ gameId, filename, contentType, size, type, category, version, visibility, user }) {
        const game = await this.resolveGame(gameId);

        // Validate payload
        this.validateUploadInput({ type, category, filename, contentType, size, version });
        ...
        const resolvedVisibility = visibility || (category === 'build' ? 'private' : 'public');
    ...
    async completeUpload({ assetId, user }) {
        ...
        try {
            ...
            console.warn(`Could not read metadata for '${asset.storageKey}':`, err.message);
        }
     ...
     */
    async startMultipartUpload({ gameId, filename, contentType, size, type, category, version, visibility, user }) {
        const game = await this.resolveGame(gameId);
    ...
    async signMultipartPart({ assetId, partNumber }) {
        ...
        if (!partNum || partNum < 1 || partNum > 10000) {
            throw new ApiError(400, 'INVALID_PART_NUMBER', 'partNumber must be between 1 and 10000');
        }
    ...
    async completeMultipartUpload({ assetId, parts }) {
        ...
        if (!Array.isArray(parts) || parts.length === 0) {
            throw new ApiError(400, 'INVALID_PARTS', 'parts array of { partNumber, etag } is required');
        }
    ...
    async abortMultipartUpload({ assetId }) {
        ...
        if (asset.uploadId) {
            try {
                ...
                console.warn(`Failed to abort multipart in R2 for key '${asset.storageKey}':`, err.message);
            }
        }
    ...
    async getDownloadUrl({ assetId, user, download = false }) {
        ...
        if (asset.visibility === 'private') {
            ...
            if (!isAdmin) {
                throw new ApiError(403, 'FORBIDDEN', 'You are not authorized to access this private asset');
            }
        }
    ...
    async deleteAsset({ assetId, user }) {
        ...
        try {
            ...
            console.error(`Failed to delete object '${asset.storageKey}' from storage:`, err.message);
        }
