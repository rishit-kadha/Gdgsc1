...
const storageConfig = {
    ...
    limits: {
        file: parseEnvInt(process.env.MAX_FILE_SIZE_GAME_BYTES, 5 * 1024 * 1024 * 1024), // 5 GB default for builds
        video: parseEnvInt(process.env.MAX_FILE_SIZE_VIDEO_BYTES, 500 * 1024 * 1024),     // 500 MB default for videos
        image: parseEnvInt(process.env.MAX_FILE_SIZE_IMAGE_BYTES, 20 * 1024 * 1024),      // 20 MB default for images
        asset: parseEnvInt(process.env.MAX_FILE_SIZE_ASSET_BYTES, 100 * 1024 * 1024),    // 100 MB default for config/other
    },
    ...
    allowedExtensions: {
        file: ['.zip', '.rar', '.7z', '.tar', '.gz', '.exe', '.dmg', '.pkg', '.apk', '.iso', '.bin'],
    ...
    categories: ['build', 'thumbnail', 'banner', 'screenshot', 'trailer', 'asset', 'other'],
    ...
    multipart: {
        thresholdBytes: parseEnvInt(process.env.MULTIPART_THRESHOLD_BYTES, 20 * 1024 * 1024), // 20 MB
